...
export function initEditor(main) {
  ...
  function fillInclude(cm, option) {
    return new Promise((accept) => {
      setTimeout(() => {
        let cur = (cm.getCursor().line = cm.getLine(cur.line).trim())
        var start = cur.ch,
          end = cur.ch
        ...
        if (line.startsWith('#include "lygia')) {
          ...
          if (result.length > 0) {
            ...
            return accept({ list: result, from: CodeMirror.Pos(cur.line, start), to: CodeMirror.Pos(cur.line, end) })
          }
        }
        return accept(null)
      }, 100)
...
export function unfocusAll(cm) {
  for (let i = 0, j = cm.getDoc().lineCount(); i <= j; i++) unfocusLine(cm, i)
}
...
export function focusAll(cm) {
  for (let i = 0, j = cm.getDoc().lineCount(); i <= j; i++) focusLine(cm, i)
}
